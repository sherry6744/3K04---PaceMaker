...
MODE = struct.pack("H",int(parameter['Mode']))
LOWER_RATE_LIMIT = struct.pack("H",int(parameter['LRL']))
#UPPER_RATE_LIMIT = struct.pack("H",int(parameter['URL']))
MAX_SENS_RATE = struct.pack("H",int(parameter['MSR']))
ATR_AMP = struct.pack("f",float(parameter['AA']))
ATR_PW = struct.pack("f",float(parameter['AP']))
ATR_SENS = struct.pack("f",float(parameter['AS']))
ARP = struct.pack("H",int(parameter['ARP']))
VENT_AMP = struct.pack("f",float(parameter['VA']))
VENT_PW = struct.pack("f",float(parameter['VP']))
VENT_SENS = struct.pack("f",float(parameter['VS']))
VRP = struct.pack("H",int(parameter['VRP']))
PVARP = struct.pack("H",int(parameter['PVARP']))
#HYES = struct.pack("H",int(parameter['H']))
#RATE_SMO = struct.pack("H",int(parameter['RS']))
ACTIVITY_THRES = struct.pack("H",int(parameter['AT']))
RECOV_TIME = struct.pack("H",int(parameter['RT']))
RESPOND_FACTOR = struct.pack("H",int(parameter['RspT']))
REACTION_TIME = struct.pack("H",int(parameter['RecT']))
